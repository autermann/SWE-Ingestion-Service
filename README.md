...
``` 
On success, the stream is running.

## API:

#### general:

   * GET [http://localhost:8082/cnc/api](http://localhost:8082/cnc/api) - gets a list of resources.

#### Sources:

  * GET [http://localhost:8082/cnc/api/sources](http://localhost:8082/cnc/api/sources) - gets all registered sources.

#### Processors:

  * GET [http://localhost:8082/cnc/api/processors](http://localhost:8082/cnc/api/processors) - gets all registered processors.

#### Sinks:

  * GET [http://localhost:8082/cnc/api/sinks](http://localhost:8082/cnc/api/sinks) - gets all registered sinks.

#### Streams:

  * GET [http://localhost:8082/cnc/api/streams](http://localhost:8082/cnc/api/streams) - gets all registered streams.

  * GET api/streams/{streamName} with `Accept` header `application/json` - gets the registered stream with name `streamName` or 404 if that stream is not found.

  * GET api/streams/{streamName}  `Accept` header `application/xml` - gets the sensorml processDescription of the registered stream with name `streamName` or 404 if the stream or its sensorML processDescription is not found.


  * register a new stream: POST against `api/streams` with `Content-Type` header `application/xml` and request body with a `AggregateProcess` description.
